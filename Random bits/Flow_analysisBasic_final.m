...
for i=1:length(GoodBetas_select)
    idx_temp=GoodClusters_goodmembers(i).idx;
    Fighandle=figure;set(Fighandle, 'Position', [10, 10, 770, 570]);
    h=histogram(categorical(idx_PerBrainRegions(idx_temp)),categorical(unique(idx_PerBrainRegions)),'Normalization','probability');ylim([0 0.3]);
...
for i=1:length(GoodBetas_select)
    idx_temp=GoodClusters_goodmembers(i).idx;
    idx_fish_temp=idx_Fish(idx_temp);
    h=nan(length(Fish_list),length(unique(RegionList_select2)));
    for fish_nb=1:length(Fish_list)    
        idx_temp_fish=idx_temp(idx_fish_temp==Fish_list(fish_nb));
        h(fish_nb,:)=histcounts(categorical(idx_PerBrainRegions(idx_temp_fish)),categorical(unique(idx_PerBrainRegions)),'Normalization','probability');
    end
    All_distrib=horzcat(All_distrib,h');
    csvwrite(strcat('D:\Pictures\processed\Flow\Basic\Figure\BrainDistrib_clusters',num2str(i),'.csv'),h*100);    
end

All_distrib_Outliers=isoutlier(All_distrib,'grubbs');
All_distrib_noOutliers=All_distrib;All_distrib_noOutliers(All_distrib_Outliers)=nan;
...
for i=1:length(GoodBetas_select)
    ...
    for fish_nb=1:length(Fish_list)    
        idx_temp_fish=idx_temp(idx_fish_temp==Fish_list(fish_nb));
        h(fish_nb,:)=histcounts(categorical(idx_PerBrainRegions(idx_temp_fish)),categorical(unique(idx_PerBrainRegions)));
    end
    All_distrib=horzcat(All_distrib,h');
    %csvwrite(strcat('D:\Pictures\processed\Flow\Basic\Figure\BrainDistrib_clusters',num2str(i),'.csv'),h*100);    
end
...
for i=1:length(RegionList_select)
    ...
    idx_temp(idx_temp==0)=nan;
    h=histogram(categorical(idx_temp),categorical(unique(idxKmeans_ZS_goodmembers_1To8)),'Normalization','probability');
    h.FaceColor = [0.5 0.5 0.5];
    h.EdgeColor = 'k';
%     set(gca,'XTick',[1:1:length(RegionList_select)]);
%     set(gca,'YTick',[0.1:0.1:0.5]);set(gca,'YTickLabel',[10:10:50]);
...
for fish_nb=1:size(Mean_allNodes_perFish,2)
    list_fish_temp=[1:1:size(Mean_allNodes_perFish,2)];
    list_fish_temp(list_fish_temp==fish_nb)=[];
    CorrelationMatrices_minusFish(:,:,fish_nb)=squeeze(nanmean(CorrelationMatrices_perFish(:,:,list_fish_temp),3));
end
