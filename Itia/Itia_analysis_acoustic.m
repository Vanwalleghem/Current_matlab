...
Stimuli=zeros(11,size(GoodCalcium,2));
...
for i=1:10
    for j=1:3
        idxStart=20*i;
        Stimuli(i,(idxStart+(j-1)*200):(idxStart+(j-1)*200)+size(GCaMP6_short,1)-1)=GCaMP6_short;        
    end    
end
idxStart=20;
for i=31:33
    Stimuli(11,(idxStart+(i-1)*20):(idxStart+(i-1)*20)+size(GCaMP6_short,1)-1)=GCaMP6_short;        
end
...
idx_rsq_sort=find([ModelResults.rsquared]>0.1 & [ModelResults.rsquared]<1);
figure;imagesc(ZS(idx_rsq_sort,:),[-1 4]);colormap hot

Stimuli2=zeros(2,size(GoodCalcium,2));
GCaMP6=[0,1.69644104899772,5.13796058542217,8.27886020152244,10.3756715204800,11.8173714529814,12.2425184714093,10.8571417354877,8.80831829681196,6.91339112244670,5.46959264663869,4.30868766622567,3.42533619066766,2.75378443486879,2.18017250852183,1.72816235135824,1.32732537295463,1.00684435500268,0.730210038304555,0.530242444093118,0.362253250339685,0.227668255288566,0.0869242416152502,0.000718266708050853,-0.0828334873368325]';
GCaMP6_short=[0,5.13796058542217,10.3756715204800,12.2425184714093,8.80831829681196,5.46959264663869,3.42533619066766,2.18017250852183,1.32732537295463,0.730210038304555,0.362253250339685]';
idxStart=20;
for i=1:33
    Stimuli2(1,(idxStart+(i-1)*20):(idxStart+(i-1)*20)+size(GCaMP6_short,1)-1)=GCaMP6_short;
    Stimuli2(2,(idxStart+4+(i-1)*20):(idxStart+4+(i-1)*20)+size(GCaMP6_short,1)-1)=GCaMP6_short;
end

ModelResults2=[];
parfor i=1:length(ZS)
    %mdl=stepwiselm(Regressor',DataSet(i,:),'linear','Criterion','adjrsquared','Intercept',false,'Upper','interactions','Verbose',0);
    mdl=stepwiselm(Stimuli2',ZS(i,:),'linear','Criterion','adjrsquared','Upper','interactions','Verbose',0);
...
idx_rsq_sort2=find([ModelResults2.rsquared]>0.1 & [ModelResults.rsquared]<1);
figure;imagesc(ZS(idx_rsq_sort2,:),[-1 4]);colormap hot
options = statset('UseParallel',1); [idxKmeans_ZS_sort2 Cmap_ZS_sort2]=kmeans(ZS(idx_rsq_sort2,:),30,'Options',options,'Distance','cityblock','Replicates',3,'MaxIter',1000,'Display','final');
...
idxKmeans_ZS=zeros(size(ZS,1),1);
idxKmeans_ZS(idx_rsq_sort2)=idxKmeans_ZS_sort2;
...
for idx=1:length(MatFiles)
    ...
    imagename=strcat(imagename{1},'_mean.tif');
    image=double(imread(imagename));image=image/max(max(image));image=image*100;
    image=uint8(image);
    image2=zeros(size(image(:,:,1)));
    image3=repmat(image,1,1,3);
