Oculomotor_Masks={};
MaskFiles=dir('D:\Pictures\processed\Itia\ROI_ANTs\Masks\*Oculomotor*.tif');
...
Oculomotor_Masks{1,1}=Brain_region;
Oculomotor_Masks{1,2}=SubRegion;
info = imfinfo(strcat('D:\Pictures\processed\Itia\ROI_ANTs\Masks\',fname));
num_images = numel(info);
PixelList=[];
for k = 1:num_images
    %transpose and rotated to align with Itia's stuff
    image_temp = imread(strcat('D:\Pictures\processed\Itia\ROI_ANTs\Masks\',fname), k, 'Info', info)';image_temp=rot90(image_temp);image_temp=image_temp>0;
...
Oculomotor_Masks{2,1}=Brain_region;
Oculomotor_Masks{2,2}=SubRegion;
info = imfinfo(strcat('D:\Pictures\processed\Itia\ROI_ANTs\Masks\',fname));
num_images = numel(info);
PixelList=[];
for k = 1:num_images
...
Oculomotor_Masks{2,3}=PixelList;
clearvars x y PixelList info num_images image_temp Brain_region SubRegion fname MaskFiles i k ans temp

load('D:\Pictures\processed\Itia\New\__ZSAll.mat')
load('D:\Pictures\processed\Itia\New\__MultiBrainRegions.mat','ROIs');
load('D:\Pictures\processed\Itia\New\__MultiBrainRegions.mat','Fish_list');
load('D:\Pictures\processed\Itia\New\__MultiBrainRegions.mat','MatFiles');

ItiaList={'Thalamus','Cerebellum','NucMLF','Semicircularis','Telencephalon','Tectum','Longitudinalis','Tegmentum','Habenula','Hindbrain','MON','Pretectum','Oculomotor'};
i=13;
regionName=ItiaList{i};
Mask=[Oculomotor_Masks{1,3} ;Oculomotor_Masks{2,3}];
...
load('D:\Pictures\processed\Itia\New\__MultiBrainRegions.mat','Stimuli');
for j=13
...
idx_temp=find(LinReg.(regionName).rsquared>0.05);
...
%Get the good ones
[temp,LinReg.(regionName).GoodBetas]=Test_Regress( LinReg.(regionName).KmeansCenter,Stimuli,LinReg.(regionName).KmeansIdx,0.3);
...
idx_g=ismember(idx_temp,GoodBet_temp([1 3]));
idx_g=find(idx_g==1);
...
figure;
counter=1;xplot=1;yplot=3;counter2=1;
for i=13
    regionName=ItiaList{i};
...
i=13;colors{i}=[0.14 1 0.14; 0.7 0.4 1];regionName=ItiaList{i};LinReg.(regionName).GoodBetas_AVG_final=LinReg.(regionName).GoodBetas_AVG([1 2]);
...
idx_temp=find(LinReg.(regionName).rsquared>0.1);
ROIs_temp=ROIsPerBrain.(regionName).ROIs(idx_temp,:);
test=ismember(ROIs_temp,Oculomotor_Masks{1,3},'rows');
