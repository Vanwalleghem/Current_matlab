...
for idx=1:length(ModelMultipower2)
    ...
    if ~isempty(temp)
        ...
        for coef_idx=2:height(coef)
            if coef.pValue(coef_idx)<0.05
                coefficients2{idx,str2num(temp{coef_idx}{1}{1})}=coef.Estimate(coef_idx);
            end
        end
    end
end
idxempty=cellfun('isempty',coefficients2);
coefficients2(idxempty)={0};
clearvars idxempty idx coef_idx coef temp
coefficients2=cell2mat(coefficients2);

idx_rsq3=find([ModelMultipower2.rsquared]>0.15);
options = statset('UseParallel',1); [idxKmeans_ZS_rsq3 Cmap_ZS_rsq3]=kmeans(ZS2(idx_rsq3,:),10,'Options',options,'Distance','cityblock','Replicates',5,'MaxIter',1000,'Display','final');
[Model_ZS3,GoodBetas_ZS3]=Test_Regress(Cmap_ZS_rsq3,Stimuli,idxKmeans_ZS_rsq3,0.4);
ZS2_rsq3=ZS2(idx_rsq3,:);
options = statset('UseParallel',1); [idxKmeans_ZS_rsq2 Cmap_ZS_rsq2]=kmeans(ZS2_rsq3,15,'Options',options,'Distance','cityblock','Replicates',5,'MaxIter',1000,'Display','final');
...
figure;imagesc(ZS(idx_neg,:));
options = statset('UseParallel',1); [idxKmeans_ZS_neg Cmap_ZS_neg]=kmeans(ZS(idx_neg,:),2,'Options',options,'Replicates',5,'MaxIter',1000,'Display','final');
figure;
for i=1:5
...
Stimuli_AVG=zeros(6,size(ZS_AVG,2));
idxStart=10;
for i=1:6    
    Stimuli_AVG(i,(idxStart+(i-1)*40):(idxStart+(i-1)*40)+size(GCaMP6,1)-1)=GCaMP6;
end
...
counter=1;
framerate=4;
x = linspace(1/framerate,size(ZS_AVG,2)/framerate,size(ZS_AVG,2));
...
for i=GoodBetas
    ...
    subplot(rows,5,counter);shadedErrorBar(x, mean(ZS_AVG2(idx_temp,:),1), std(ZS_AVG2(idx_temp,:),1,1));axis([0 60 -1 4]);
    subplot(rows,5,counter+1);imagesc(ZS(idx_temp,:),[-1 4]); colormap hot
    subplot(rows,5,counter+2);histogram(idx_Plane(idx_temp));
    subplot(rows,5,counter+3);histogram(idx_Position(idx_temp),[0.5:1:3.5]);h = gca;h.XTick=[1 2];h.XTickLabel={'ELO','ERO'};
...
for i=1:length(GoodBetas)    
    ...
    parfor j=1:length(idx_temp)
        ...
        temp=corrcoef(Cmap_ZS_rsq3(GoodBetas(i),:), ZS2(idx_temp(j),:));
        corr_temp(j)=temp(1,2);
    end    
...
for i=GoodBetas
    ...
    subplot(rows,5,counter);shadedErrorBar(x, mean(GoodClusters_goodmembers(counter2).ZS,1), std(GoodClusters_goodmembers(counter2).ZS,1,1));axis([0 60 0 5]);
    subplot(rows,5,counter+1);imagesc(GoodClusters_goodmembers(counter2).ZS,[0 5]); colormap hot
    subplot(rows,5,counter+2);histogram(idx_Plane(idx_temp));
    subplot(rows,5,counter+3);histogram(idx_Position(idx_temp),[0.5:1:2.5]);h = gca;h.XTick=[1 2];h.XTickLabel={'ELO','ERO'};
    subplot(rows,5,counter+4);histogram(idx_Fish(idx_temp));%h = gca;h.XTickLabel={'1','2','3','4','5','6','7'};
    counter=counter+5;
    counter2=counter2+1;
end
