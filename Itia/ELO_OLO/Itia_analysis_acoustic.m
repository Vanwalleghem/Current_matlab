...
Stimuli=zeros(2,size(GoodCalcium,2));
...
for i=1:5
    ...
    Stimuli(2,(idxStart+4+(i-1)*40):(idxStart+4+(i-1)*40)+size(GCaMP6,1)-1)=GCaMP6;
end
...
for i=1:length(MatFiles)	
    ...
    if strfind(name,'30Hz')
        Fish=1;
    elseif strfind(name,'300Hz')
        Fish=2;
    elseif strfind(name,'500Hz')
        Fish=3;
    else
        Fish=4;
    end
