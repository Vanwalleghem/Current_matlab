...
options = statset('UseParallel',1); [idxKmeans Cmap]=kmeans(ZS,150,'Options',options,'Distance','correlation','Replicates',5,'MaxIter',1000,'Display','final');

options = statset('UseParallel',1); [idxKmeans_ZS2 Cmap_ZS2]=kmeans(ZS,200,'Options',options,'Distance','correlation','Replicates',5,'MaxIter',1000,'Display','final');
options = statset('UseParallel',1); [idxKmeans_ZS Cmap_ZS]=kmeans(ZS,200,'Options',options,'Distance','cityblock','Replicates',5,'MaxIter',1000,'Display','final');

Threshold=0.5;
[Model_ZS,GoodBetas]=Test_Regress(Cmap_ZS,flow,idxKmeans_ZS,Threshold);
[Model_ZS2,GoodBetas2]=Test_Regress(Cmap_ZS2,flow,idxKmeans_ZS2,Threshold);
[Sorted_rsq,order_rsq]=sort([Model_ZS.rsquared]);
...
idxKmeans_ZS_goodmembers=zeros(1,size(Calcium,1));
for i=1:length(GoodBetas_select)
...
GoodClusters_goodmembers(i).ZS=GoodClustersData(i).DF(find(GoodClustersData(i).CorrCoef>=Threshold),:);
