...
i=8;
figure;counter=1;
for Threshold=[0.2:0.1:1]    
    subplot(3,3,counter);plot(x,mean(GoodClustersData(i).ZS(find(GoodClustersData(i).CorrCoef>=Threshold),:),1));
    title(num2str(length(find(GoodClustersData(i).CorrCoef>=Threshold))));
    counter=counter+1;
end
