...
spatial_factor=1
idx_xy=None
base_name=os.environ["TMPDIR"]+"/"+fname[0].split('/')[-1][:-4]
min_mov = cm.load(fname[0], subindices=range(100)).min()
...
mc = MotionCorrect(fnames[0],min_mov,max_shifts = [5,5],dview=dview,splits_rig = 5, nonneg_movie = True)    
mc.motion_correct_rigid(save_movie=True)
border_to_0 = np.ceil(np.max(mc.shifts_rig)).astype(np.int)
fname_new = cm.save_memmap([mc.fname_tot_rig], base_name='base_name', border_to_0=border_to_0)
